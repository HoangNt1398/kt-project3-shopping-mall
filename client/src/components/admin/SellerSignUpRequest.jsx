...
const SellerSignUpReqeust = () => {
  ...
  function sellerRequestAxios() {
    //!        페이지네이션 구현해야하는데 아직임
    return axios
      .get(`${process.env.REACT_APP}/admin?page=1&size=1`, noBodyOptions)
      .then((res) => {
  ...
  const deleteSellerRequest = (sellerId) => {
    return axios
      .patch(`${process.env.REACT_APP}/admin/rejected/${sellerId}`, null, noBodyOptions)
      .then((res) => {
        console.log(`res.data:`);
        console.log(res.data);
  ...
  const acceptSellerRequest = (sellerId) => {
    return axios
      .patch(`${process.env.REACT_APP}/admin/approval/${sellerId}`, null, noBodyOptions)
      .then((res) => {
        console.log(`res.data:`);
        console.log(res.data);
        window.location.reload();
